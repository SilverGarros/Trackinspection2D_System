...
std::optional<std::vector<SensorData>> csvSensorData(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (tokens.size() >= 6) {
            record.index = std::stoi(tokens[0]);
            record.timestamp = tokens[1];
            record.speed1 = std::stod(tokens[2]);
            record.value1 = std::stoi(tokens[3]);
            record.speed2 = std::stod(tokens[4]);
            record.value2 = std::stoi(tokens[5]);

            //std::cout << "Debug: index=" << record.index
            //    << ", value1=" << record.value1
            //    << ", value2=" << record.value2 << std::endl;
...
bool parseImageName(const std::string& imageName, int& imgIndex, int& part, int& totalParts) {
...
/**
 * 解析字符串中的类别列表（兼容逗号分隔，自动去除空格）
 * @param input 输入字符串（格式如 "GD, BM, ZC"）
 * @return 解析后的类别列表向量
 */
std::vector<std::string> parseClassList(const std::string& input) {
...
void collect_target_images(const std::vector<T>& results, const std::string& img_root_path,
    ...
    for (const auto& result : results) {
        ...
        std::string cam_name = (result.Camera == "L") ? "左相机_railhead_stretch" : "右相机_railhead_stretch";
        //std::cout << "转换前 cam_name: " << cam_name << std::endl;
         //修正乱码与中文的对应关系
        if (cam_name == "鍙崇浉鏈") {
            cam_name = "右相机";  // 原逻辑此处写反
        }
        else if (cam_name == "宸︾浉鏈") {
            cam_name = "左相机";  // 原逻辑此处写反
        }
...
void merge_results_to_db(
    ...
    const char* insert_sql = R"(
        INSERT INTO result (
            DefectType, Camera, ImageName, X, Y, H, W, Confidence, Area, Points, PointsArea,Position
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    )";
    ...
    for (auto& dr : results) {
        ...
        if (!imageNameSuffix.empty() && !dr.ImageName.empty()) {
        ...
        // Position
        if (dr.Position != -1)
            g_sqliteLoader.sqlite3_bind_double_fn(stmt, 12, dr.Position);
        else
            g_sqliteLoader.sqlite3_bind_null_fn(stmt, 12);
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::string collect_target_classes = XmlLoader::get_value_from_xml(
            xml_path, "CollectTargetClasses", "GD,BM,ZC"
        );
            ...
            // 主处理逻辑
                ...
                for (const auto& cam : { L_cam_name, R_cam_name }) {
                    ...
                    futures.push_back(std::async(std::launch::async, [&, cam]() {
                        ...
                        // 统一计算缺陷详细位置
                        if (csv_file_ready && calculate_DefectPosition_or_not) {
